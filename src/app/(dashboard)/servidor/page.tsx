...
export default async function ServidorPage() {
  ...
  // Fetch all tasks without dimension filter
  ...
  return (
    <div className="h-full flex flex-col p-8 bg-[#121212] overflow-y-auto">
      <header className="mb-8 flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-bold text-[#e5e2e1]">Servidor Geral</h2>
          ...
          <div className="flex-1 overflow-y-auto">
            {tasks && tasks.length > 0 ? tasks.map((task) => (
              <div key={task.id} className="grid grid-cols-12 gap-4 p-4 border-b border-[#2D2D2D] hover:bg-[#252525] transition-colors text-sm items-center">
                <div className="col-span-4 font-medium text-[#e5e2e1] truncate">{task.nome}</div>
                <div className="col-span-2">
                  <span className="text-[#A0A0A0] text-xs font-medium border border-[#2D2D2D] px-2 py-1 rounded bg-[#131313]">
                    {task.dimensao || '-'}
                  </span>
                </div>
                <div className="col-span-2">
                  <span className={`px-2 py-1 text-[11px] rounded-md font-medium border ${
                    task.status.toLowerCase().includes('andamento') 
                ...
                <div className="col-span-2 text-[#A0A0A0]">
